...
// bortoman select kora category
$current_cat = isset($_GET['cat']) ? $_GET['cat'] : 'All';

// =========================================
// ১. Product Delete Logic
// =========================================
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_product'])) {
    ...
    if ($stmt->execute([$title, $category, $price, $stock, $image, $gallery, $description, $id])) {
...
// Edit er jonne product data fetch kora (jodi kono product edit e click kora hoy)
$edit_data = null;
...
<style>
    ...
    .btn-action { background: rgba(255,255,255,0.1); color: #fff; padding: 6px 12px; border-radius: 5px; text-decoration: none; font-size: 12px; font-weight: bold; transition: 0.3s; margin-right: 5px; border: 1px solid rgba(255,255,255,0.2);}
    .btn-action:hover { background: var(--accent-orange); border-color: var(--accent-orange); }
    
    .btn-danger { background: rgba(231, 76, 60, 0.2); color: #e74c3c; border: 1px solid #e74c3c; padding: 6px 12px; border-radius: 5px; text-decoration: none; font-size: 12px; font-weight: bold; transition: 0.3s; }
    ...
    <form action="manage_products.php" method="POST">
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input type="text" name="image" value="<?= $edit_data ? htmlspecialchars($edit_data['image']) : '' ?>" placeholder="Paste image link here" required>
            </div>
        </div>
...
<div class="form-group" style="margin-top: 15px;">
    <label>Gallery Images (Optional - Separate multiple links with comma)</label>
    <textarea name="gallery" rows="2" placeholder="e.g. link1.jpg, link2.jpg, link3.png"><?= $edit_data ? htmlspecialchars($edit_data['gallery']) : '' ?></textarea>
</div>
                ...
                <?php foreach($products as $product): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="manage_products.php?cat=<?= urlencode($current_cat) ?>&delete=<?= $product['id'] ?>" class="btn-danger" onclick="return confirm('Are you sure you want to delete this product?');">
...
</div> <div class="modal-overlay" id="addCatModal">
    <div class="modal-box">
...
<script>
    ...
    // New Category Submit Logic
    function submitNewCategory() {
        var catName = document.getElementById('new_category_name').value.trim();
        if(catName !== "") {
            // Re-directs to the page with the new category selected, 
            // so the user can directly add a product under it.
